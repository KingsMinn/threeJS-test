...
function RoomEnvironment() {
	...
	// -y (바닥 조명 추가 - 하단 반사 개선) - 더 밝게!
	const light7 = new THREE.Mesh( geometry, createAreaLightMaterial( 60 ) );
	light7.position.set( 0.0, -10.0, 0.0 );
	light7.scale.set( 8.0, 0.1, 8.0 );
	scene.add( light7 );

	// 바닥 추가 조명들 (여러 방향에서) - 더 밝게!
	const light8 = new THREE.Mesh( geometry, createAreaLightMaterial( 45 ) );
	light8.position.set( 5.0, -8.0, 5.0 );
	light8.scale.set( 3.0, 0.1, 3.0 );
	scene.add( light8 );

	const light9 = new THREE.Mesh( geometry, createAreaLightMaterial( 45 ) );
	light9.position.set( -5.0, -8.0, -5.0 );
	light9.scale.set( 3.0, 0.1, 3.0 );
	scene.add( light9 );

	const light10 = new THREE.Mesh( geometry, createAreaLightMaterial( 35 ) );
	light10.position.set( 0.0, -12.0, 0.0 );
	light10.scale.set( 12.0, 0.1, 12.0 );
	scene.add( light10 );
